...
3. **localhost:5050 (포트번호) 실행**
 - 웹을 실행시킨다.
4. **로그인**
 - 통합정보시스템에서 가져온 정보를 src/databases/users.json에 저장하였다고 가정하였다.
   따라서, 파일 안의 정보인 "ID: 이현식 PSWORD: 1234", "ID: 김하롱 PSWORD: 1111"로 로그인을
   하면, 올바른 회원 정보로 인식되어 수강과목 목록으로 넘어가게 된다.
